...
Controller.prototype.authenticate = async function (req, res) {
  ...
  if (!payload && req.headers.authorization && req.headers.authorization.includes('Bearer ')) {
    ...
    if (valid) payload = valid
    log.todo(payload, 'FIXME payload of token')
  }
...
/**
 * Authenticate for the ccdb entrypoint on Traefik
 *
 * This handles authentication for cross-cluster database
 * connections. These go through a dedicated entrypoint in
 * Traefik because they are not accessible to regular users.
 * These requests come with a JWT that we verify here.
 *
 * @param {object} req - The request object from Express
 * @param {object} res - The response object from Express
 */
