 ...
 */
export function Group(id = false) {
 ...
 */
Group.prototype.list = async function () {
 ...
 */
Group.prototype.isAvailable = async function (id) {
 ...
 */
Group.prototype.create = async function (id, description = '') {
  if (!id) return false
  /*
 ...
 */
Group.prototype.update = async function (id, description = '') {
  if (!id) return false
  // Run query
 ...
 * @return {object} updated - true if it the group is updated, false if not
 */
Group.prototype.addGroupToGroups = async function (id, groups = []) {
  if (!Array.isArray(groups)) return false
  const results = []
  for (const group of groups) results.push(await this.addGroupToGroup(id, group))

  return true
}

Group.prototype.addGroupToGroup = async function (id, group) {
  const isUnknown = await this.isAvailable(group)
  if (isUnknown) {
    log.warn(`Not adding group ${id} to ${group} because the target group does not exist`)
    return false
  }
  const createsLoop = await this.detectGroupLoops(id, group)
  if (createsLoop) {
...
Group.prototype.removeFromGroup = async function (member, group) {
...
Group.prototype.addHostToGroup = async function (host, group) {
  /*
   * Insert into the database
   */
  await utils.db.write(
 ...
 */
Group.prototype.detectGroupLoops = async function (id, group) {
  const q = `
 ...
 */
Group.prototype.loadGroupsHierarchy = async function () {
  // Run the query
...
ORDER BY path;
  ...
  return status === 200 ? this.buildGroupsTree(resultsAsList(result)) : false
}

Group.prototype.buildGroupsTree = function (items) {
 ...
 */
Group.prototype.read = async function (id) {
 ...
 */
Group.prototype.loadGroupHostMembers = async function (id) {
  const [status, result] = await utils.db.read(
 ...
 */
Group.prototype.loadGroupGroupMembers = async function (id) {
  const [status, result] = await utils.db.read(
    // j = join table, s = source table
 ...
 * @return {object} list - The list of (group) members
 */
Group.prototype.loadGroupMemberOf = async function (id) {
  const [status, result] = await utils.db.read(
    // j = join table, s = source table
 ...
 * @return {object} list - The list of (host) members
 */
Group.prototype.loadGroupMembers = async function (id) {
  /*
 ...
 */
Group.prototype.delete = async function (id = false) {
