 ...
 */
export const schemaViolation = (error, res) => res.status(400).send({ errors: error.details.map(err => err.message) }).end()
 ...
 */
export const setupNotPossible = (res) => res.status(401).send({
 ...
 * @param {res} res - The Express response object
 */
export const setupTokenInvalid = (res) => res.status(401).send({
