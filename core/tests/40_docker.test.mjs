...
describe('Core Docker Active Tests', () => {
   ...
   *     sshOptions: {},
   *     protocol: 'http',
   *     socketPathCache: '/var/run/docker.sock'
   *   },
   *   id: '4150302a720c31dfee0a8c3667c426dba543ba2dde6ef9c1482d4395208a8824',
   *   defaultOptions: { ... }
   * }
   */
  ...
  it(`Should GET /docker/containers/:id`, async () => {
    const result = await core.get(`/docker/containers/${store.test_container}`)
    const d = result[1]
    assert.equal(Array.isArray(result), true)
    assert.equal(result.length, 3)
    assert.equal(result[0], 200)
    ...
    assert.equal(d.Config.Hostname, 'morio_core_test')
  ...
  it(`Should GET /docker/containers/:id/stats`, async () => {
    const result = await core.get(`/docker/containers/${store.test_container}/stats`)
    const d = result[1]
    assert.equal(Array.isArray(result), true)
    assert.equal(result.length, 3)
    assert.equal(result[0], 200)
  ...
  it(`Should GET /docker/images/:id`, async () => {
    const result = await core.get(`/docker/images/${store.test_image}`)
    const d = result[1]
    assert.equal(Array.isArray(result), true)
    assert.equal(result.length, 3)
    assert.equal(result[0], 200)
  ...
  it(`Should GET /docker/images/:id/history`, async () => {
    const result = await core.get(`/docker/images/${store.test_image}/history`)
    const d = result[1]
    assert.equal(Array.isArray(result), true)
    assert.equal(result.length, 3)
    assert.equal(result[0], 200)
   ...
   *     socketPathCache: '/var/run/docker.sock'
   *   },
   *   id: 'bd47a40f1d0871facfd4a204f03d163f96b9bc6a76c7b2ac0ccf4b38533b0c70'
   * }
   */
  it(`Should POST /docker/network`, async () => {
    const result = await core.post(`/docker/network`, {
  ...
  it(`Should GET /docker/networks/:id`, async () => {
    const result = await core.get(`/docker/networks/${store.test_network}`)
    const d = result[1]
    assert.equal(Array.isArray(result), true)
    assert.equal(result.length, 3)
    assert.equal(result[0], 200)
    ...
    assert.equal(d.Name, 'morio_test_network')
...
describe('Core Docker Container State Tests', () => {
  for (const state of ['start', 'pause', 'unpause', 'stop', 'restart', 'kill']) {
    ...
    it(`Should PUT /docker/containers/:id/${state}`, async () => {
      const result = await core.put(`/docker/containers/${store.test_container}/${state}`)
      const d = result[1]
      assert.equal(Array.isArray(result), true)
      assert.equal(result.length, 3)
