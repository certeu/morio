    ...
    <PageWrapper {...props} layout={OidcLayout} header={false}>
      <div className="p-8 w-full max-w-2xl mx-auto mt-12">
        <div className="border border-secondary p-0 rounded-lg shadow">
          ...
          <div className="p-4">
            <h4 className="px-4">
              You are currently signed in to Morio as{' '}
              <span className="text-warning font-bold">
                {account.user}
                <small>@</small>
                {account.provider}
              </span>{' '}
              using the <span className="text-warning font-bold">{account.role}</span> role
            </h4>
            ...
            <div className="p-4">
              <form action={`/interaction/${uid}/login`} method="POST">
                ...
                <button type="submit" className="btn btn-secondary px-12 w-full">
                  Continue
                </button>
              </form>
              <div className="grid grid-cols-2 gap-2 mt-2">
                <button className="btn btn-neutral btn-outline w-full" onClick={logout}>
                  Logout
                </button>
                <Link className="btn btn-error btn-outline w-full" href="/">
                  Cancel
                </Link>
              </div>
            </div>
