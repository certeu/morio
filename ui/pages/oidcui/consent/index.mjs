      ...
      <div className="p-8 w-full max-w-2xl mx-auto mt-12">
        <div className="border border-secondary p-0 rounded-lg shadow">
          ...
          <div className="p-4 px-8">
            <h4>
              <span className="text-warning">{name}</span> by{' '}
              <span className="text-warning">{by}</span> wants to access your Morio account
            </h4>
            <p className="mt-4">
            ...
            <ul className="list list-inside ml-4">
              <li className="flex flex-row items-center gap-2">
                <BoolYesIcon /> Read data from the{' '}
              ...
              <li className="flex flex-row items-center gap-2">
                <BoolYesIcon /> Act on your behalf, with the{' '}
                <b className="text-warning">{account.role}</b> role
              </li>
            </ul>
            <div className="py-4">
              <h6 className="flex flex-row items-center gap-4 my-4">
                ...
                <div>
                  Make sure that you trust <span className="text-warning font-bold">{name}</span> by{' '}
                  <span className="text-warning font-bold">{by}</span> before you allow this
                </div>
              </h6>
              <p>
                You may be sharing sensitve information with this website or app. Please ensure that
                you can trust its developer(s) before you allow this access.
              </p>
              <form action={`/interaction/${uid}/confirm`} method="POST">
                <input type="hidden" name="prompt" value="consent" />
                {scopes.map((scope) => (
                  <input key={scope} type="hidden" name="grantedScopes" value={scope} />
                ))}
                {claims.map((claim) => (
                  <input key={claim} type="hidden" name="grantedClaims" value={claim} />
                ))}
                <button type="submit" className="btn btn-secondary w-full">
                  Allow
                </button>
              </form>
              <Link className="btn btn-error btn-outline mt-2 w-full" href="/">
                Cancel
              </Link>
            </div>
          </div>
