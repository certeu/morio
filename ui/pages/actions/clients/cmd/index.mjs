...
export const getStaticProps = () => ({
  props: {
    ...
    page: ['actions', 'clients', ['cmd', 'Send Client Commands']],
  },
})
...
const ClientCommandBox = () => {
  ...
  const runCommand = async (cmd) => {
    ...
    const result = await api.sendClientCommand(cmd, impacted > 0 ? Object.keys(selected) : false)
    if (result[1] === 200 && result[0]?.id) {
      setLoadingStatus([true, 'Redirecting to command status page', true, true])
      router.push(`/actions/clients/cmd/${result[0].id}`)
    } else {
  ...
  return (
    <div>
      ...
      <ul className="list list-inside mt-4 ml-4 list-disc mb-4">
        {clients
          ...
          .map((client) => (
            <li key={client.id}>
              <button
                className={`font-mono btn ${
                  selected[client.id] ? 'btn-success' : 'btn-neutral btn-outline'
      ...
      <div className="flex flex-row gap-2 items-center border-warning border rounded p-1 px-2 my-2 text-sm bg-warning bg-opacity-20">
        <TipIcon className="h-5 w-5 text-warning" /> This command will target{' '}
        <b>{impacted ? impacted : 'all'}</b> clients
      </div>
    </div>
  )
}
...
const CommandWarning = ({ cmd, selected, impacted, runCommand, clearModal }) => (
  <div>
    ...
    <p>
      Do you want to send the <b>{cmd}</b> command to <b>{impacted > 0 ? impacted : 'all'}</b>{' '}
      clients?
    </p>
    <div className="flex flex-rwo items-center gap-2">
      <button onClick={() => runCommand(cmd)} className="btn btn-primary">
        Send Command
      </button>
      <button onClick={clearModal} className="btn btn-outline">
        Cancel
      </button>
    </div>
    {impacted > 0 ? (
      <ul className="list list-inside flex flex-row flex-wrap gap-1 items-center">
        ...
        {Object.values(selected).map((client) => (
          <li key={client.id} className="font-mono text-xs">
            {client.fqdn || client.name || client.id}
          </li>
        ))}
      </ul>
    ) : null}
