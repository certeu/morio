  ...
  const install = (
    <ModalWrapper closeHandler={() => setModal(false)}>
      <div className="">
        ...
        <p className="mb-2">
          To add the <b>{data.title}</b> client module to your Morio setup, use these settings:
        </p>
        <DarkCode>
          {`preseed:
    git:
    ...
    <div className="tailwind">
      ...
      <H1>
        <div className="flex flex-row flex-wrap gap-2 items-center w-full justify-between">
          <div>
            {data.title}
            <span className="block md:inline opacity-50 text-base pl:0 md:pl-4">
              a Morio Client Module
            </span>
          </div>
      ...
      <p>
        This module bundles configuration for the following agents:
        {Object.keys(data)
          .sort()
          .filter((key) => key !== 'title')
          .map((agent, i) => (
            <span className="px-1" key={agent}>
              <a className={linkClasses} href={`#${agent}`}>
                {agent}
              </a>
            ...
            <H2>
              <div className="flex flex-row flex-wrap gap-2 items-center w-full justify-between">
                <div>
                  ...
                  <span className="text-sm font-medium pl-2 opacity-70">
                    (template for {agentNames[agent]})
                  </span>
                </div>
                <div>
              ...
              <>
                ...
                <div className="pl-2">
                  {Object.entries(data[agent].moriodata.vars).map(([varKey, varInfo]) => (
                    <div key={varKey}>
                      <H4>
                        <span className="pr-2">{varKey}:</span>
      ...
      <div className="max-w-5xl mx-auto mb-12">
        ...
        <H1>
          <div className="flex flex-row flex-wrap gap-2 items-center w-full justify-between">
            <div>
              {data.title}
              <span className="opacity-50 text-base pl-0 md:pl-4 block md:inline">
                a Morio Settings Overlay
              </span>
            </div>
  ...
  const install = (
    <ModalWrapper closeHandler={() => setModal(false)}>
      <div className="">
        <H3>Install instructions</H3>
        <p className="mb-2">
          To add the <b>{data.title}</b> stream processor to your Morio setup, use these settings:
        </p>
        <DarkCode>
          {notes?.frontMatter?.install_raw
            ? 'test'
            : `preseed:
    git:
      moriohub:
    ...
    <div className="tailwind">
      <div className="max-w-5xl mx-auto mb-12">
        ...
        <a
          ...
          href={`https://github.com/certeu/moriohub/tree/develop/processors/${data.title}`}
        >
...
const HomeIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6 -mb-2 pr-2">
    <path
...
export const H3 = ({ children }) => <h3 className="text-xl font-bold mb-2 mt-4">{children}</h3>
export const H4 = ({ children }) => <h4 className="text-base font-bold mb-0 mt-2">{children}</h4>
