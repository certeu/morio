...
sidebar_label: preseed # skip-spellcheck
...
In practice, we utilise the key under the `preseed.git` settings as the `id`.
...
:::tip
You can find this under **Settings** → **General** on your repository page in
the GitLab UI.
:::
...
:::tip Learn more about preseeding and overlays
For all details about preseeding, refer to the [Preseeding
Guide](/docs/guides/settings/preseed).
:::
