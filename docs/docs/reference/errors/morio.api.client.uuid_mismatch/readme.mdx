---
title: "morio.api.client.uuid_mismatch" # skip-spellcheck
tags:
 - error

---
<!-- MORIO_AUTO_GENERATED_CONTENT_STARTS - Manual changes made below will be overwritten skip-spellcheck -->
__Client UUID mismatch__ - The client UUID is not matched with a cluster node UUID which has the same FQDN
<!-- MORIO_AUTO_GENERATED_CONTENT_ENDS - Manual changes made above will be overwritten skip-spellcheck -->


<!-- MORIO_AUTO_GENERATED_CONTENT_STARTS - Manual changes made below will be overwritten skip-spellcheck -->
## Example response

```json
{
  "type": "https://morio.it/docs/reference/errors/morio.api.client.uuid_mismatch",
  "status": 400,
  "title": "Client UUID mismatch",
  "detail": "The client UUID is not matched with a cluster node UUID which has the same FQDN"
}
```
<!-- MORIO_AUTO_GENERATED_CONTENT_ENDS - Manual changes made above will be overwritten skip-spellcheck -->
