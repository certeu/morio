...
authorization is handled in Morio.   
...
ways to comminicate with a Morio instance:
...
This is the user identity that is used to apply authorization.
...
- Including the cluster UUID ensures that clients certificates are linked to the
  Morio instance that generated them.
...
Authorization is the process of establishing what a user can (or cannot) do.

### Kafka Authorization

When connecting over the Kafka protocol, authorization is governed by the
...
:::tip
Keep in mind that customizes the broker configuration to extract the value of
...
This describes the type of resouce. It can be one of `TOPIC`, `CLUSTER`,
`GROUP`, or `TRANSACTIONAL_ID`. 

You can almost certainly ignore all but `TOPIC` since that is the reource type
...
Authorization for HTTP is divided in two different approaches:
...
Note that roles are commulative, so every higher role can do everything the
...
| `engineer` | Allows decrypting Morio data | IT Engineers, or staff doing Morio troubleshooting |
...
Refer to [the gitops guide](/docs/guides/gitops/) for more details.
:::
