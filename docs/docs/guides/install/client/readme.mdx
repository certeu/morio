...
The Morio client uses one folder for configuration, one for data, and another one for
...
If you get a bunch of output about `apt` packages management, you are on an APT-based system.
...
:::warning
The next steps are not safe to run on a Morio Cluster Node if you want to
remove only the client and keep the Cluster Node
:::
