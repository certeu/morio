...
Unless you have a specific reason you need the _bare_ client, you should use
the client packages made available by your Morio instance.

:::warning The bare client is not avialable yet
...
Run the following command from an account with sudo rights:

```sh
curl -k https://{{ MORIO_CLUSTER_FQDN }}/install/ | bash
```
