...
Morio utilizes one folder for configuration, one for data, and another one for
...
<Tabs>
<TabItem value="production" label="Production Channel">
```sh
...
Let's see how we can accmplish that.
...
If you want to clean up after stopping Morio, make sure to remove all its contaiers:
