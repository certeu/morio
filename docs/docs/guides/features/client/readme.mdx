...
Stricty speaking, you don't even need the Morio client. You can install the
...
### High level abstraction: Centralized client management {#fleet}
...
or managing these vars in a centralized way.
...
The control plane only comes into play when using [centralized client management](#fleet).
In that scenarion, these are the additional data flows:
...
daemonizes and connects to the _broker service_ where it listens for (consumes)
...
To enrol a clint, you _join_ it to a cluster passing it the cluster FQDN:
...
Once the cluster knows what modules to assign to this client, you can _pull_ the configation:
