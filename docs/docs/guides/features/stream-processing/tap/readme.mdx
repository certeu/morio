...
- You can add additional settings that the user can levarage to control how
...
As you can see, this results is a more more polishes experience for the suer
who gets a little info about what this stream processor does.
...
one to disable them without having to unload them.
...
The way it works is that Morio's tap service will subscribe to all topics your
specified in [the `topics` settings](#topics).
Each time a new message arrives on one of these topics, it will a function you
...
We have strived to keep the boilerplate to a minimum. You merely need to define
a function, and make it the default export.  The function receives 3
...
is received on the topic(s) we have subscribed to, `data` will hols one such
...
expect, although the actual `data` will obviously different for you.
...
helper object that provides you with various tools to you can leverage inside
...
Here's a brief overview of what is provides by tools, for a full list, please
refer to [the stream  processing reference documentation](#fixme).

- `tools.axios`: Holds an instance of the Axios HTTP client for NodeJS
- `tools.get`: Holds a copy of the lodash get
- `tools.set`: Holds a copy of the lodash set
- `tools.unset`: Holds a copy of the lodash unset
- `tools.cache`: Holds various helpers to add data to the Morio cache
  ...
  - `tools.cache.healthcheck`: Caches healthcheck data
...
- `tools.inventory`: Helper methods to mamage the Morio inventory
- `tools.ipaddr`: A copy of the ipaddr library to wrangle IP addresses
- `tools.node`: A helper method for logging that side-steps teh potential logging snowball effect
- `tools.valkey`: An instance of the ValKey/Redis client for NodeJS for low-level cache access
...
- `tools.extract`: Helper methods to generate various types of info from data received from RedPanda/Kafka
  ...
  - `tools.extract.check`: Extracts the full healthcheck URL from the data
  - `tools.extract.host`: Extracts the host ID from the data
  - `tools.extract.id`: Extracts the message ID from the data
  - `tools.extract.metricset`: Extracts the (name of the) metricset from the data
...
- `tools.log`: A logger object, an instance of the Pinki logger. Beware of the snowball effect when logging.
...
- `tools.produce`: Various methods to publish data to a RedPanda/Kafka topic
...
The third and final paramter, `topic` holds the name of the topic the `data`
was sourced from.  If your stream processor only subscribes to a single topic,
this information is not very useful. But if you have a stream processors that
...
does -- this allows you to differentiate your logic based on the topic the
message came from.
...
Then update your preseed settings. Here is an example that loads all stream processesors from moriohub:
...
the stream processor get picked up agan by the same stream processor,
ever-increasing the amound of log data flowing through the system.
