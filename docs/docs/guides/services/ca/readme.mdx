...
certainly a _nice to have_ that Morio comes with one out of the box.
...
Refer to the [`/ca/certificate`](/docs/reference/apis/api#tag/cryptography/paths/~1ca~1certificate/post) API endpoint for details.
...
to encrypt communication or use mTLS. Often, these people do not have an 
internal CA, or have one but it requires a written request in duplicate signed
by the CISO before a certificate can be issued.
