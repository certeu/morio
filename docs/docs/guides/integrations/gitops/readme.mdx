...
- When you inevitably need to add _secrets_ to your settings, you have a
  chicken/egg situtation where you do not want to store secrets in git, but
...
- `USERNAME`: The username that the `PERSONAL_ACCESS_TOKEN` belows to. This
  needs to be a user that can clone the respository.
