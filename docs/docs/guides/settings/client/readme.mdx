...
Before we dive into configuration of the Morio client, let us take a moment to
ensure we have a good mental modal if what the client is and does.
...
When you install the Morio client, you will have the various beats agents on
your system, and can interact with them directly if you so wish.
...
As Morio uses several specialized agents under the hood, we have further
facilitated management of the client's configuration through a higher level
configuration abstraction that leverages _templates_ and _vars_.

#### Morio client templates

A Morio client template is a modular piece of agent configuration. It can
describe what logfiles to collect (for Filebeat), what files to watch for
changes (for Auditbeat), or what metrics to collect (for Metricbeat).
...
effort to take the drudge out of configurating observability.
...
If at this point you are wondering _but you don't know where I store my NGINX
logs_ then thank you for providing the perfect segway to our next point.
...
a specific agent you specify.  It's a convencience method that allows you to
...
writeable by the root user.
:::

### morio audit/logs/metrics

Running any of these commands will pass-through your command options to the
repective agent. It has the benefit that the agent will be invoked with the
...
You can use this every time you need low-level access one of the agents.

## Client configuration folder

All the high-level tempates and vars work is mapped to file operations in the Morio client configuration folder.
...
files in the `vars.d` folder.  If you would like to forego the who
templates/vars system altogether and configure the various agents yourself, you
can drop your `.yml` files in the various configuration folders. The Morio
client only uses `.yaml` files, and will ignore all `.yml` files.
