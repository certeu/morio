...
service mostly because we had experience with it, it allows for plenty
different output and inputs, and since we're using the Beats agents, 
it seemed like a good fit.

But it was not a passionate choice for Logstash. It runs on the JVM -- which
we don't love -- its _LSCL_ configuration langauge is undocumented, and it's
hard to troubleshoot.

We had been keeping our eye on [Vector](https://vector.dev/) for a while now,
but without a good reason to make the change, it felt like we always had
more urgent matters to attent to than replace something that already worked.

But when Logstash started giving us grief, we decided the time had come
...
We average this out out to 1.5k/s because that's easier to reason about,
