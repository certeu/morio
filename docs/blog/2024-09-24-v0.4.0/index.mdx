...
some things we wanted to see in this release that haven't been implemeted yet.
...
While we could have backported our fix -- which was a simple as upgrading 
...
accept out release cadence to accelerate, as we'll be publishing patch releases
with smaller updates. This will also allow us to streamline our release 
workflows and artifact build pipelines.
...
there is authorization with SASL. Morio will automatically create a `root` and
...
authorization where we were unable to extract the user principal from the
certificate. And issue for which a fix has so far eluded us. 

Once we resolve this, we will switch authorization back to mTLS and drop SASL.
...
in Morio's _managment API_ backed by our _DB service_.

This is a very simply KV store -- think a poor man's etcd -- but if it fully 
distributed on clusted deployments.
...
We've added rate limiting to the management API, as part of our effors to make
...
This is the best way since authentication of requests with a JWT are much more
efficient.
...
sign in step adds friction and actually would be slower than making the initial
request with credentials.

We've made changes to allow basic authentnication for the `apikey` and `mrt`
identity providers, as those are the ones typically used in an automated way.

### Prevent generating certificates that would grant elevated privileges

Once we start using mTLS for authorization on the Kafka API, the certificate's
...
- [api.repo.morio.it](https://apt.repo.morio.it/)
...
This allows you then install Morio as any regular packages, and you will also get your
updates that way.
...
changelog](https://github.com/certeu/morio/blob/develop/CHANGELOG.md).
