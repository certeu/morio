...
To some extend, we can foresee such flexibility and incorporate it in our
...
To that extend, we currently have 3 different ways you can add your own
...
pipelines, thus allowing you to re-use any existing Logstash pipelines you may
have.
...
- `v0.6`: This `0.6` release in the `stable` channel.
- `v0.6-canary`: This `0.6` release in the `canary` channel.
- `v0.6-testing`: This `0.6` release in the `testing` channel.
- `stable`: This most recent release in the `stable` channel.
- `canary`: This most recent release in the `canary` channel.
- `testing`: This most recent release in the `testing` channel.
- `latest`: This most recent release in the `stable` channel (this is an alias
...
Debian Bookworm. We have since recreated these APT repositories, and now they
are not tied to a specific Debian version, or even Debian for that matter.

Which means you can use them on different Debian version, or even on Debian
...
This allows you to change certain Moriod presets, much like you would do
...
message to help identity this issue.
...
We've made changes to the various life cycles methods of the services in Morio
core, and in doing so have significantly reduced the time needed for Morio to
reconfigure itself, or restart itself.
