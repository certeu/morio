import { api, brokerNodes, fqdn, hostName } from './utils.mjs'
...
describe('Anonymous Routes Tests', () => {
  ...
  it(`Should GET /status`, async () => {
    ...
    // state
    assert.equal(typeof d.state, 'object')
    ...
    assert.equal(d.core.info.production, true)
    // core.status.cluster
    ...
    // core.status.nodes
    assert.equal(typeof d.core.status.nodes[brokerNodes], 'object')
    // core.nodes
    assert.equal(typeof d.core.nodes, 'object')
    const uuid = Object.keys(d.core.nodes).pop()
    assert.equal(d.core.nodes[uuid].fqdn, fqdn)
    assert.equal(d.core.nodes[uuid].hostname, hostName)
