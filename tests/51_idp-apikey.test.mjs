...
describe('API Key Tests', () => {
  for (const field of ['name', 'expires', 'role']) {
    ...
    it(`Should not POST /apikey (missing ${field})`, async () => {
      ...
      const result = await api.post(`/apikey`, data, false, false, true, jwt)
  ...
  it(`Should POST /apikey`, async () => {
    ...
    const result = await api.post(`/apikey`, keys.key1, false, false, true, jwt)

    assert.equal(result[0], 200)
    const d = result[1]
    assert.equal(d.name, keys.key1.name)
    assert.equal(d.status, 'active')
    assert.equal(d.created_by, `local.${sharedStorage.get('accounts.user').username}`)
  ...
  it(`Should GET /apikeys`, async () => {
    ...
    const result = await api.get(`/apikeys`, false, false, true, jwt)
  ...
  it(`Should POST /apikeys/:key/rotate`, async () => {
    ...
    const result = await api.patch(
      `/apikeys/${store.keys.key1.key}/rotate`,
      undefined,
      false,
      false,
      true,
    ...
    assert.equal(d.updated_by, `local.${sharedStorage.get('accounts.user').username}`)
  ...
  it(`Should POST /apikeys/:key/disable`, async () => {
    ...
    const result = await api.patch(
      `/apikeys/${store.keys.key1.key}/disable`,
      undefined,
      false,
      false,
      true,
  ...
  it(`Should POST /apikeys/:key/enable`, async () => {
    ...
    const result = await api.patch(
      `/apikeys/${store.keys.key1.key}/enable`,
      undefined,
      false,
      false,
      true,
  ...
  it(`Should GET /whoami (JWT in Bearer header)`, async () => {
    const result = await api.get(`/whoami`, false, false, true, store.keys.key1)
    assert.equal(result[0], 200)
    const d = result[1]
  ...
  it(`Should DELETE /apikeys/:key`, async () => {
    const result = await api.remove(`/apikeys/${store.keys.key1.key}`)
