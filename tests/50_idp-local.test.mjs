...
import { store, accounts, api, validateErrorResponse, sharedStorage } from './utils.mjs'
...
describe('API Create Account Tests', () => {
  ...
  it(`Should GET /accounts`, { timeout }, async () => {
    ...
    const result = await api.get(`/accounts`, false, false, true, jwt)
    const d = result[1]
  ...
  it(`Should POST /activate-account`, async () => {
    ...
    const result = await api.post(`/activate-account`, data, false, false, true, jwt)
    assert.equal(result[0], 200)
    const d = result[1]
    assert.equal(typeof d, 'object')
...
describe('API Create Account Tests', () => {
  ...
  it(`Should not POST /login (missing provider)`, async () => {
    ...
    const result = await api.post(`/login`, data, false, false, true, jwt)
    validateErrorResponse(result, errors, 'morio.api.schema.violation')
  })
  ...
  it(`Should POST /login`, async () => {
    ...
    assert.equal(d.data.available_roles.length, 4)
    assert.equal(d.data.available_roles[0], 'user')
    assert.equal(d.data.highest_role, 'engineer')
  ...
  it(`Should GET /whoami (JWT in cookie)`, async () => {
    const result = await api.get(`/whoami`, false, false, true, null, store.accounts.user2.jwt)
    ...
    assert.equal(d.available_roles.length, 4)
    assert.equal(d.available_roles[0], 'user')
    assert.equal(d.highest_role, 'engineer')
  ...
  it(`Should GET /whoami (JWT in Bearer header)`, async () => {
    const result = await api.get(`/whoami`, false, false, true, { jwt: store.accounts.user2.jwt })
  ...
  it(`Should GET /token (JWT in Cookie)`, async () => {
    const result = await api.get(`/token`, false, false, true, null, store.accounts.user2.jwt)
    assert.equal(result[0], 200)
  ...
  it(`Should GET /token (JWT in Bearer header)`, async () => {
    const result = await api.get(`/token`, false, false, true, { jwt: store.accounts.user2.jwt })
    assert.equal(result[0], 200)
    assert.equal(typeof result[1].jwt, 'string')
  })
